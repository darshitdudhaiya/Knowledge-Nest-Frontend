...
// import Jwt from "custom/Jwt.js";;
const Sidebar = ({ open, onClose }) => {
  const userTokenDetails = JSON.parse(environment.userTokenDetails);

  const filteredRoutes = routes.filter(route => {
    if (userTokenDetails.Role === "Admin") {
      return route.layout === "/admin";
    } else if (userTokenDetails.Role === "Faculty") {
      return route.layout === "/faculty";
    } else if (userTokenDetails.Role === "Student") {
